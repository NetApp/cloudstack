...
public class StorageSystemDataMotionStrategy implements DataMotionStrategy {
    ...
    private void verifyFormatWithPoolType(ImageFormat imageFormat, StoragePoolType poolType) {
        if (imageFormat != ImageFormat.VHD && imageFormat != ImageFormat.OVA && imageFormat != ImageFormat.QCOW2 &&
                ...
                StoragePoolType.FiberChannel == poolType || StoragePoolType.OntapSAN == poolType))) {
            throw new CloudRuntimeException(String.format("Only the following image types are currently supported: %s, %s, %s, %s (for PowerFlex, FiberChannel and OntapSAN)",
